...
class HaarFaceDetector:
    ...
    def returnFacePosition(self, inputImg, 
        ...
        for position in order:
            ...
            if(runFrontal==True and position==1):
                self._findFrontalFace(np.array(inputImg), frontalScaleFactor, minSizeX, minSizeY)
            ...
            if(runFrontalRotated==True and position==2):
                rows, cols = np.array(inputImg).shape[:2]
                M = cv2.getRotationMatrix2D((cols/2,rows/2),rotationAngleCCW,1) #30 degrees ccw rotation
                inputImgRot = cv2.warpAffine(np.array(inputImg), M, (cols,rows))
            ...
            if(runLeft==True and position==4):
                self._findProfileFace(np.array(inputImg), leftScaleFactor, minSizeX, minSizeY)
                if(self.is_face_present == True):
                    self.face_type = 4
                    return (self.face_x, self.face_y, self.face_w, self.face_h)
            #Cascade: right profiles
            if(runRight==True and position==5):
                flipped_inputImg = cv2.flip(np.array(inputImg),1) 
    ...
    def returnMultipleFacesPosition(self, inputImg,
        ...
        #Cascade: frontal faces
        if(runFrontal==True):
            faces = self._findMultipleFrontalFaces(np.array(inputImg), frontalScaleFactor, minSizeX, minSizeY)
            if(self.is_face_present == True):
...
if __name__ == "__main__":
    ...
    facedetector.returnMultipleFacesPosition(inputImg = "/home/kayadev-gpu-2/Pictures/Screenshotforface.png")